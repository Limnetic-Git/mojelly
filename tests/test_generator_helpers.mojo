...
def test_strip_comments() raises:
    assert_equal(strip_source_comment("var a = 10 # this is a comment"), "var a = 10 ")
    assert_equal(strip_source_comment("var b = 20"), "var b = 20")
    assert_equal(strip_source_comment('var str = "#not_comment" # comment'), 'var str = "#not_comment" ')
...
def test_split_arguments() raises:
    ...
    var complex_args = split_top_level_arguments("a: List[Int, Float], b: Dict[String, Int]")
...
def test_wrapper_name_and_generation() raises:
    assert_equal(generated_dto_wrapper_name("submit_user"), "__mojelly_dto_submit_user")
    assert_equal(generated_dto_wrapper_name("handle.user!"), "__mojelly_dto_handle_user_")
    ...
    assert_true("def __mojelly_dto_user_handler(req: HTTPRequest) -> HTTPResponse:" in wrapper_code)
    assert_true("try_deserialize[UserDTO]" in wrapper_code)

    var routes_code = generate_routes_code(routes, handler_dtos)
    assert_true('router_ptr[].add("POST", "/user", __mojelly_dto_user_handler)' in routes_code)
